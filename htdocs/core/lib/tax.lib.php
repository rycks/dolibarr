...
function tax_by_thirdparty($type, $db, $y, $date_start, $date_end, $modetax, $direction, $m = 0, $q = 0)
{
	...
	if (($date_start || $date_end) && (!empty($y) || !empty($m) || !empty($q))) {
		dol_print_error('', 'Bad value of input parameter for tax_by_thirdparty');
	}
			...
			while ($assoc = $db->fetch_array($resql)) {
				...
				if ($assoc['rowid'] != $oldrowid) {       // Si rupture sur d.rowid
					...
					if ($situationInvoice) {
						...
						$list[$rate_key]['localtax1'] += ($assoc['total_localtax1'] - $assocPrev['total_localtax1']);
						$list[$rate_key]['localtax2'] += ($assoc['total_localtax2'] - $assocPrev['total_localtax2']);
					} else {
